...
describe("elysia-connect", () => {
        ...
        const app = new Elysia()
            ...
            .get('/test1', async () => 'Test 1', {
                async beforeHandle(context) {
                    const resp = await context.elysiaConnect(createNoOpMiddleware(), context);
                    if (resp) return resp;
                }
            })
            .get('/test2', async () => 'Test 2', {
                async beforeHandle(context) {
                    const resp = await context.elysiaConnect(createMiddleware({body: 'Middleware 2'}), context);
                    if (resp) return resp;
                }
            });
    ...
    it("should support path matching", async () => {
        const app = new Elysia()
            .use(
                elysiaConnect(
                    createMiddleware({body: 'Hello middleware'}),
                    {
    ...
    it("should support any methods", async () => {
        const app = new Elysia()
            .use(elysiaConnect((req, res, next) => {
...
type ConnectMiddleware = (req: IncomingMessage, res: ServerResponse, next: NextFn) => void
...
function createRespMiddleware(
    ...
    return (req: IncomingMessage, res: ServerResponse, next: NextFn) => transformResponse(res, next)
}
...
>(app: App, expected: Record<Path, Body>, inits?: Partial<Record<Path, RequestInit>>) {
